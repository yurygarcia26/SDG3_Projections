Boost_glm_function <- function(region_col, training_data, testing_data, ..., hyper_tune=TRUE, cv=TRUE) {
  ...
  # Find best hyper parameters for Boosted GLM model
  cat("Hyper parameter tunning for Boosted GLM ... \n")
  ...
  if (hyper_tune) {
    tunegrid <- expand.grid(mstop = c(100, 500, 1000), prune = c(0, 0.01, 0.1, 0.2))
  } else {
    tunegrid <- expand.grid(mstop = 1000, prune = 0.1)
  }
  ...
  cat("Feature Selection for Boosted GLM ... \n")
