...
Forecast_Function <- function(Outcome_name, region, diff=FALSE){
  ...
  if(diff == TRUE){
    covariate_projections_file <- paste("././Projections/Projection_Data/Covariable_Projections",
                                        Outcome_name,
  ...
  saveRDS(new_model, paste("././Projections/Models_2019/best_model_", region, "_", Outcome_name, ".rds", sep=""))
...
for (Outcome_name in Outcome_names) {
  ...
  figure <- grid_plot
  ggsave(paste("././Projections/Projection_Results/All_Predictions_", Outcome_name,
               "_Region.pdf",sep=""), figure, width = 15, height = 20) 
  write.xlsx(list_of_forecasts, paste("././Projections/Projection_Results/Forecasts_", 
                                Outcome_name, "_Region.xlsx", sep=""))
